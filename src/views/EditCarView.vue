    ...
    <el-card class="max-w-[880px] m-auto p-0" body-class="!p-0">
      ...
      <div class="px-5 mt-4">
        <Form @submit="onSubmit">
          ...
          <button class="mt-3 px-2 py-1 bg-blue-500 text-white">updtae</button>
        </Form>
      </div>
    </el-card>
  </main>
...
const { errors, defineInputBinds, setErrors, handleSubmit, setValues } = useForm({
...
onMounted(async() => {
  try {
    ...
    picked.value = response.data.state;
  } catch (error) {
    console.log(error);
  }
});
const onSubmit = handleSubmit(async(values, { resetForm }) => {
  ...
  await store.dispatch('carStore/updateCar', { id, data });
